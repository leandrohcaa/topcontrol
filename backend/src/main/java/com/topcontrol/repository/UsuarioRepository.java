...
public interface UsuarioRepository extends BaseRepository<Usuario, Long>, UsuarioRepositoryCustom {
	...
	@Query("select u from Usuario u join fetch u.usuarioNegocioList un "
			+ " left join fetch un.negocio n " + " where u.usuario = :user and u.senha = :password")
	Usuario loginOwner(@Param("user") String user, @Param("password") String password);
	@Query("select u from Usuario u join fetch u.usuarioNegocioList un "
			+ " left join fetch un.negocio n " + " where u.usuario = :user ")
	Usuario loginOwner(@Param("user") String user);

	@Query("select u from UsuarioNegocio un join un.usuarioList u "
			+ " where un.usuario.id = :usuarioId and un.negocio.id = :negocioId")
	List<Usuario> fetchUsuarioListInUsuarioNegocio(@Param("usuarioId") Long usuarioId, @Param("negocioId") Long negocioId);
	@Query("select u from UsuarioNegocio un join un.clienteList u "
			+ " where un.usuario.id = :usuarioId and un.negocio.id = :negocioId")
	List<Usuario> fetchClienteListInUsuarioNegocio(@Param("usuarioId") Long usuarioId, @Param("negocioId") Long negocioId);
